...
def get_match_data_datail() -> Tuple[List, Dict]:
    ...
    while not match_id_queue.empty():
        ...
        for player_puuid in match_data["info"]["participants"]:
    ...
    # fail to save data send sqs
    if match_id_queue.qsize() > 0:
        while not match_id_queue.empty():
            match_id = match_id_queue.get()
            match_id_sqs.send_message(
                DelaySeconds=10,
                MessageBody=(
                    f"{match_id}"
                )
